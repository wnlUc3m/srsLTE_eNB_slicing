...
bool enb::init(all_args_t *args_)
{
  ...
  srsenb::s1ap_interface_rrc *s1ap_rrc[NUM_SLICES];
  srsenb::gtpu_interface_rrc *gtpu_rrc[NUM_SLICES];
  srsenb::gtpu_interface_pdcp *gtpu_pdcp[NUM_SLICES];
  for (int slice = 0; slice < NUM_SLICES; ++slice){
    s1ap[slice] = new srsenb::s1ap();
    s1ap_rrc[slice] = s1ap[slice];

    gtpu[slice] = new srsenb::gtpu();
    gtpu_rrc[slice] = gtpu[slice];
    gtpu_pdcp[slice] = gtpu[slice];
  }
  ...
  pdcp.init(&rlc, &rrc, gtpu_pdcp, &pdcp_log);
  rrc.init(&rrc_cfg, &phy, &mac, &rlc, &pdcp, s1ap_rrc, gtpu_rrc, &rrc_log);
  
  //Init dedicated layers
  printf("\n  --  S1C : %s MME: %s S1U : %s SPGW: %s\n", args->enb.s1ap.s1c_bind_addr.c_str(), args->enb.s1ap.mme_addr.c_str(), args->enb.s1ap.gtp_bind_addr.c_str(), args->enb.s1ap.sgw_addr.c_str());
  printf("  --  S1C : %s MME: %s S1U : %s SPGW: %s\n", args->enb.s1ap2.s1c_bind_addr.c_str(), args->enb.s1ap2.mme_addr.c_str(), args->enb.s1ap2.gtp_bind_addr.c_str(), args->enb.s1ap2.sgw_addr.c_str());

  //FIXME: Create an array of S1AP arguments and insert initialization in a loop.
  s1ap[0]->init(args->enb.s1ap, &rrc, &s1ap_log);
  gtpu[0]->init(args->enb.s1ap.gtp_bind_addr, args->enb.s1ap.sgw_addr, &pdcp, &gtpu_log);

  s1ap[1]->init(args->enb.s1ap2, &rrc, &s1ap_log);
  gtpu[1]->init(args->enb.s1ap2.gtp_bind_addr, args->enb.s1ap2.sgw_addr, &pdcp, &gtpu_log);
...
bool enb::get_metrics(enb_metrics_t &m)
{
  ...
  s1ap[0]->get_metrics(m.s1ap);
  s1ap[1]->get_metrics(m.s1ap);
  m.running = started;  
  return true;
}
