...
void parse_args(all_args_t *args, int argc, char* argv[]) {
  ...
  common.add_options()
    ...
    ("enb.name2",         bpo::value<string>(&args->enb.s1ap2.enb_name)->default_value("srsenb01"), "eNodeB Name")
    ...
    ("enb.mme_2_addr",      bpo::value<string>(&args->enb.s1ap2.mme_addr)->default_value("127.0.0.1"),"MME addr (second core)")
    ("enb.s1c_2_bind_addr", bpo::value<string>(&args->enb.s1ap2.s1c_bind_addr),		                   "eNB IP addr for S1-C (second core)")
    ("enb.sgw_2_addr",      bpo::value<string>(&args->enb.s1ap2.sgw_addr),                            "S-GW IP addr (second core)")
    ("enb.gtp_2_bind_addr", bpo::value<string>(&args->enb.s1ap2.gtp_bind_addr),                       "eNB IP addr for S1-U (second core)")    
  ...
  // Convert hex strings
  {
    ...
    sstr2 << std::hex << vm["enb.enb_id"].as<std::string>();
    sstr2 >> args->enb.s1ap2.enb_id;
  ...
  }
  {
    ...
    // gines
    std::stringstream sstr2;
    sstr2 << std::hex << vm["enb.tac"].as<std::string>();
    sstr2 >> args->enb.s1ap2.tac;
  }
  ...
  // gines
  if(!srslte::string_to_mcc(mcc, &args->enb.s1ap2.mcc)) {
    cout << "Error parsing enb.mcc:" << mcc << " - must be a 3-digit string." << endl;
  }
  if(!srslte::string_to_mnc(mnc, &args->enb.s1ap2.mnc)) {
    cout << "Error parsing enb.mnc:" << mnc << " - must be a 2 or 3-digit string." << endl;
  }
