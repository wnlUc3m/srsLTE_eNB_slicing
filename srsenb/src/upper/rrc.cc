               ...
               srslte::log* log_rrc)
{
  ...
  // Initialize map that links <plmn_id : core>
  //  * plmn_id = 1 --> core_0
  //  * plmn_id = 2 --> core_1
  network_core[1] = 0;
  network_core[2] = 1;
...
{ 
  ...
  for (int slice = 0; slice < NUM_SLICES; ++slice){
    if (s1ap[slice]->user_exists(rnti)){
      if (!s1ap[slice]->user_link_lost(rnti)){
  ...
  /*if (s1ap[0]->user_exists(rnti)) {
    if (!s1ap[0]->user_link_lost(rnti)) {
      rrc_log->info("Removing rnti=0x%x\n", rnti);
      rem_user_thread(rnti);
    }
  } else {
...
void rrc::rem_user(uint16_t rnti)
{
  ...
  if (users.count(rnti) == 1) {
    ...
    for (int slice = 0; slice < NUM_SLICES; ++slice){
      gtpu[slice]->rem_user(rnti);  
    }
    //gtpu[0]->rem_user(rnti);
...
void rrc::activity_monitor::run_thread()
{
  while(running) 
  {
    ...
    for(std::map<uint16_t, ue>::iterator iter=parent->users.begin(); rem_rnti == 0 && iter!=parent->users.end(); ++iter) {
      ...
      if (u->is_timeout()) {
        parent->rrc_log->info("User rnti=0x%x timed out. Exists in s1ap=%s\n", rnti, parent->s1ap[0]->user_exists(rnti)?"yes":"no");
        parent->rrc_log->info("User rnti=0x%x timed out. Exists in s1ap=%s\n", rnti, parent->s1ap[1]->user_exists(rnti)?"yes":"no");
        rem_rnti = rnti;        
      }      
    }    
    pthread_mutex_unlock(&parent->user_mutex);
    if (rem_rnti) {
      if (parent->s1ap[0]->user_exists(rem_rnti)) {
        parent->s1ap[0]->user_inactivity(rem_rnti);
      } else {
...
{
  parent->rrc_log->info("UE PARSE_UL_DCCH\n"); 

  set_activity();
  
  LIBLTE_RRC_UL_DCCH_MSG_STRUCT ul_dcch_msg;
  bzero(&ul_dcch_msg, sizeof(LIBLTE_RRC_UL_DCCH_MSG_STRUCT));
  parent->rrc_log->info("1.1 size=%lu\n", pdu->N_bytes); 
  ...
  switch(ul_dcch_msg.msg_type) {
    ...
    case LIBLTE_RRC_UL_DCCH_MSG_TYPE_UL_INFO_TRANSFER:
      ...
      parent->s1ap[parent->users[rnti].current_slice]->write_pdu(rnti, pdu);
      break;
...
void rrc::ue::handle_rrc_con_setup_complete(LIBLTE_RRC_CONNECTION_SETUP_COMPLETE_STRUCT *msg, srslte::byte_buffer_t *pdu)
{
  ...
  if(has_tmsi){
    parent->s1ap[parent->network_core[msg->selected_plmn_id]]->initial_ue(rnti, pdu, m_tmsi, mmec);
    parent->users[rnti].current_slice = 0;
    //this->parent->s1ap[0]->initial_ue(rnti, pdu, m_tmsi, mmec);
  }else {
    if (parent->users[rnti].current_slice == -1){
      parent->users[rnti].current_slice = 0;
    }
    else{
      parent->users[rnti].current_slice++;
    }
    parent->s1ap[parent->network_core[msg->selected_plmn_id]]->initial_ue(rnti, pdu);
  }
  state = RRC_STATE_WAIT_FOR_CON_RECONF_COMPLETE;
...
bool rrc::ue::setup_erabs(LIBLTE_S1AP_E_RABTOBESETUPLISTCTXTSUREQ_STRUCT *e)
{
  for(uint32_t i=0; i<e->len; i++) {
    ...
    parent->gtpu[0]->add_bearer(rnti, lcid, erabs[id].teid_out, &(erabs[id].teid_in));
    parent->gtpu[1]->add_bearer(rnti, lcid, erabs[id].teid_out, &(erabs[id].teid_in));
...
void rrc::ue::notify_s1ap_ue_ctxt_setup_complete()
{
  ...
  parent->s1ap[parent->users[rnti].current_slice]->ue_ctxt_setup_complete(rnti, &res);
}
...
void rrc::ue::notify_s1ap_ue_erab_setup_response(LIBLTE_S1AP_E_RABTOBESETUPLISTBEARERSUREQ_STRUCT *e)
{
  ...
  parent->s1ap[parent->users[rnti].current_slice]->ue_erab_setup_complete(rnti, &res);
}
...
void rrc::ue::send_connection_reconf(srslte::byte_buffer_t *pdu)
{
  ...
  // parent->erab_info.msg is the DedicatedInfoNas pdu and hence, inside we have the assigned IP address.
  //    - size = 74 bytes
  //      - 9 bytes DedicatedInfoNas header
  //      - 1 byte GPRS Timer
  //      - 7 bytes TAI list
  //      - 44 bytes ESM message container (Assigned IP addr is inside)
  //      - 13 bytes GUTI
  std::string str_ip;
  std::stringstream sstm;
  sstm << static_cast<int>(parent->erab_info.msg[36]) << "." << static_cast<int>(parent->erab_info.msg[37]) << "." << static_cast<int>(parent->erab_info.msg[38]) << "." << static_cast<int>(parent->erab_info.msg[39]);
  str_ip = sstm.str();

  parent->pdcp->store_assigned_ip(rnti, str_ip, current_slice);
