...
public:
  ...
  std::map<double,int> network_core;
  std::map<string,int> src_ip_to_net_core;
  //std::map<string,int> assigned_core;
...
private: 
  ...
  rlc_interface_rrc    *rlc;
  pdcp_interface_rrc   *pdcp;
  gtpu_interface_rrc   *gtpu[NUM_SLICES];
  s1ap_interface_rrc   *s1ap[NUM_SLICES];
  srslte::log          *rrc_log;
